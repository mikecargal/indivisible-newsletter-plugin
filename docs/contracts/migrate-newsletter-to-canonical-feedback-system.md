...
| C1 | Saving settings reports each rejected/coerced value — invalid IMAP encryption, invalid post status, dropped qualified-sender emails, an invalid webmaster email — via a `.ids-alert-error` validation banner; a fully-valid save shows a `.ids-alert-success` confirmation, never an unconditional WP "Settings saved." | php (test-settings-validation-feedback.php); sign-off |
...
(`indivisible-design-system` CSS + `ids-confirm-modal` on the settings page).
...
**Fixture:** a settings-page HTML fixture for the B1 jsdom test (the Recent
Newsletters table's Reprocess form + button), generated from
`indivisible_newsletter_render_recent_newsletters_section()` so the test exercises
the real rendered markup rather than a hand-built stand-in.
